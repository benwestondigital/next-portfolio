...
On the home page, to convert the user's inputted location into coordinates I had to first use the Google Autocomplete API to get a full address from the user's input, and use the place id returned from this to subsequently call the Google Places API, which returned the coordinates needed to query the MongoDB database, which then returned the cities with a similar climate. Google Maps has two APIs for different purposes – one for client side, and one for server side. I spent an evening getting one to work, only for me to realize I had implemented the wrong one and needed the server side API. This really helped me to learn to read the documentation and understand what I need before I start writing code.
